/**
 * APNs client — worker-side copy.
 *
 * This mirrors `api/src/services/apnsClient.ts`. Both workspaces are
 * intentionally kept self-contained (no cross-workspace relative imports,
 * no shared dep on `jose`, no new npm packages) so each Cloud Run service
 * image stays lean. When behaviour changes, update both files.
 *
 * The worker is the primary caller in production — the api enqueues push
 * jobs on the `push-notifications` queue and the worker actually delivers
...
async function singleSend(
  options: SendApnsPushOptions & { jwt: string },
