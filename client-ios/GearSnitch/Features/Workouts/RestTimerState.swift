...
final class RestTimerState: ObservableObject {
    ...
    init(duration: Int, tickInterval: TimeInterval = 1.0) {
        ...
        // clamped at the property setter) pass in-range values. The
        // earlier defensive clamp silently rewrote sub-10s test
        // fixtures and was blocking unit tests from driving short
        // deterministic durations (`testDoneStateForOverlay` /
        // `testNudgeOnCompleteIsNoOp`).
